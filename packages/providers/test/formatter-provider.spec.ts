...
describe('FormatterProvider', () => {
  ...
  describe('formatInstrument()', () => {
    it('should format a currency according to a locale and an instrument', () => {
      expect(formatter.formatInstrument(10_000, 'EUR', 'en-USD')).to.equal('€10,000.000');
      expect(formatter.formatInstrument(10_000, 'EUR', 'fr-FR')).to.equal('10 000,000 €');
    });
  });

  describe('formatQuote()', () => {
    it('should format a quote according to a locale and an instrument', () => {
      expect(formatter.formatQuote(1.4255, 'EUR', 'en-USD')).to.equal('€1.42550');
    });
  });
});
